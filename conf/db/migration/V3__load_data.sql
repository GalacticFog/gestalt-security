...
INSERT INTO user_group(group_id,group_name) VALUES
  ('a9RCjvdRcQ8sbtV5sjs9wlDo', 'callfairy_users'),
...
INSERT INTO account_x_group(group_id,account_id) VALUES
  ((SELECT group_id FROM user_group WHERE group_name = 'callfairy_users'), (SELECT account_id FROM user_account WHERE username = 'brad')),
  ((SELECT group_id FROM user_group WHERE group_name = 'callfairy_users'), (SELECT account_id FROM user_account WHERE username = 'sy')),
  ((SELECT group_id FROM user_group WHERE group_name = 'callfairy_users'), (SELECT account_id FROM user_account WHERE username = 'anthony')),
  ((SELECT group_id FROM user_group WHERE group_name = 'callfairy_users'), (SELECT account_id FROM user_account WHERE username = 'chris')),
...
INSERT INTO app_user_store(app_id,group_id) VALUES
  ((SELECT app_id FROM app WHERE app_name = 'CallFairyScheduler'), (SELECT group_id FROM user_group WHERE group_name = 'callfairy_users'));

-- ----------------------------------------------------------------------------
-- Rights
-- ----------------------------------------------------------------------------
INSERT INTO right_grant(grant_id,grant_name,grant_value,account_id,group_id,app_id) VALUES
  ('WZIpVpyn6ftFEnM1Z241qS2v', 'callfairy:user_access',          NULL, NULL, (SELECT group_id from user_group WHERE group_name = 'callfairy_users'),         (SELECT app_id from app WHERE app_name = 'CallFairyScheduler')),
