...
object SDKAccountFactory extends AccountFactoryDelegate {
	...
	override def createAccount(dirId: UUID, username: String, description: Option[String], email: Option[String], phoneNumber: Option[String],
		...
		AccountFactory.createAccount(dirId, username, description, email, phoneNumber,firstName, lastName, hashMethod, salt, secret, disabled).map { APIConversions.accountModelToApi(_) }
	}
	...
	override def updateAccount(account: GestaltAccount, patches: Seq[PatchOp]): Try[GestaltAccount] = {
		UserAccountRepository.find(account.id) match {
			case Some(uar) => AccountFactory.updateAccount(uar, patches).map { APIConversions.accountModelToApi(_) }
	...
	override def updateAccountSDK(account: GestaltAccount, update: GestaltAccountUpdate): Try[GestaltAccount] = {
		UserAccountRepository.find(account.id) match {
			case Some(uar) => AccountFactory.updateAccountSDK(uar, update).map { APIConversions.accountModelToApi(_) }
			case None => throw new ResourceNotFoundException(resource = account.id.toString, message = "Error finding account for update", developerMessage = "Error finding repository account for update")
		}
	}
	...
	override def getAppAccount(appId: UUID, accountId: UUID): Option[GestaltAccount] = AccountFactory.getAppAccount(appId, accountId).map { APIConversions.accountModelToApi(_) }

	override def listEnabledAppUsers(appId: UUID): List[GestaltAccount] = AccountFactory.listEnabledAppUsers(appId).map { APIConversions.accountModelToApi(_) }
	...
	override def queryShadowedDirectoryAccounts(dirId: Option[UUID], nameQuery: Option[String], phoneQuery: Option[String], emailQuery: Option[String]): List[GestaltAccount] = {
		AccountFactory.queryShadowedDirectoryAccounts(dirId, nameQuery, emailQuery, phoneQuery).map { APIConversions.accountModelToApi(_) }
	}
